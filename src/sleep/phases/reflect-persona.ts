...
import { z } from "zod";
import { db, messages, personaAddenda } from "../../db/index.js";
...
export async function reflectPersona(ctx: DreamContext): Promise<void> {
  ...
  try {
    const r = await generateObject({
      ...
      prompt: [
        ...
        `Only include entries you are at least ${MIN_CONFIDENCE}% confident in. Prefer fewer, higher-confidence entries.`,
  ...
  for (const a of qualified) {
    ...
    if (inserted && changeId != null) {
      await db.execute(
        sql`update sleep_cycle_changes set target_id = ${String(inserted.id)} where id = ${changeId}`
      );
    }
